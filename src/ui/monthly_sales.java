...
public class monthly_sales extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jPanel1.add(jLabel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(522, 182, -1, -1));
        jPanel1.add(startDate, new org.netbeans.lib.awtextra.AbsoluteConstraints(611, 167, 136, 32));

        jLabel4.setFont(new java.awt.Font("Tahoma", 1, 14)); // NOI18N
        jLabel4.setText("End Date");
        jPanel1.add(jLabel4, new org.netbeans.lib.awtextra.AbsoluteConstraints(815, 182, -1, -1));
        jPanel1.add(enddate, new org.netbeans.lib.awtextra.AbsoluteConstraints(896, 167, 136, 32));
        ...
        jPanel1.add(jScrollPane1, new org.netbeans.lib.awtextra.AbsoluteConstraints(335, 395, -1, 283));
        ...
        jPanel1.add(jScrollPane2, new org.netbeans.lib.awtextra.AbsoluteConstraints(797, 395, -1, 283));
...
// Set up the model for the customer table
        ...
        customer_table_2.setModel(customerModel);  // Set the model to your customer_table
...
// Converting to Calendar objects to compare months and years
        Calendar startCal = Calendar.getInstance();
        startCal.setTime(utilStartDate);

        Calendar endCal = Calendar.getInstance();
        endCal.setTime(utilEndDate);

// Validate if the selected dates are in the same month and year
        if (startCal.get(Calendar.YEAR) != endCal.get(Calendar.YEAR)
                || startCal.get(Calendar.MONTH) != endCal.get(Calendar.MONTH)) {
            JOptionPane.showMessageDialog(null, "Please select a date range within the same month.");
...
// date format to convert it 
        ...
        try {
            ...
            customerStmt.setDate(1, sqlStartDate);  // Set the start date parameter
            customerStmt.setDate(2, sqlEndDate);    // Set the end date parameter
            ...
            // Query for product report data (product_name, quantity, priceperunit, totalprice) based on the selected date range
            String productQuery = "SELECT product_name, SUM(qty) AS quantity, priceperunit AS price_per_unit, SUM(totalprice) AS total_sold "
                    + "FROM products WHERE date BETWEEN ? AND ? GROUP BY product_name, priceperunit";
            PreparedStatement productStmt = con.prepareStatement(productQuery);
            productStmt.setDate(1, sqlStartDate);  // Set the start date parameter
            productStmt.setDate(2, sqlEndDate);    // Set the end date parameter
            ...
            if (!productDataFound) {
                JOptionPane.showMessageDialog(null, "No products have been sold for the selected date range.");
            } else {
